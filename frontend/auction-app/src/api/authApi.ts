...
export const authApi = {
    ...
    refresh: (refreshToken: string) =>
        api.post<AuthResponse>('/auth/refresh', { refreshToken } satisfies { refreshToken: string }),

    verifyUser: (data: VerifyRequest) =>
    ...
    verifyPasswordReset: (data: VerifyRequest) =>
        api.post<string>('/auth/password-reset/verify', data),

    resetPassword: (data: ResetPasswordRequest) =>
        api.post<string>('/auth/password-reset/confirm', data),
};