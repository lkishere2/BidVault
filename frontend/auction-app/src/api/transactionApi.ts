...
export const transactionApi = {
    getUserTransactions: (page = 0, size = 10) => api.get<Page<TransactionResponse>>(`/transaction/me?page=${page}&size=${size}`),
    ...
    getAllTransactionRequests: (page = 0, size = 10) => api.get<Page<TransactionResponse>>(`/transaction/all?page=${page}&size=${size}`),
