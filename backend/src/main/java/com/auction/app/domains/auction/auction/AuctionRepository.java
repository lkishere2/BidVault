...
public interface AuctionRepository extends JpaRepository<Auction, Long> {
        ...
        @Query(value = "SELECT a.id FROM Auction a WHERE a.seller.id = :sellerId", countQuery = "SELECT COUNT(a.id) FROM Auction a WHERE a.seller.id = :sellerId")
        ...
        @Query(value = "SELECT DISTINCT a.id FROM auctions a " +
                        ...
                        "AND (CAST(:endTime AS TIMESTAMP) IS NULL OR a.end_time <= :endTime)", countQuery = "SELECT COUNT(DISTINCT a.id) FROM auctions a "
                                        +
                                        "LEFT JOIN products p ON a.product_id = p.id " +
                                        "WHERE (" +
                                        "    (CAST(:status AS VARCHAR) IS NULL AND a.status IN ('UPCOMING', 'ACTIVE')) "
        ...
        @Query("SELECT a FROM Auction a JOIN FETCH a.seller JOIN FETCH a.product WHERE a.status = :status ORDER BY a.bidCount DESC")
        List<Auction> findTopAuctionsByBidCount(@Param("status") AuctionStatus status, Pageable pageable);
